...
$out = Join-Path $dataDir "head-rewrite-preview-v8-$stamp.csv"
...
# 모든 업체 TITLE에 지역명 + 업체명을 반드시 포함
$shopTitleTemplates=@(
'{L} {V} | 코스·가격·운영정보 | 몽라운지',
'{V} | {L} 코스와 영업시간 안내 | 몽라운지',
...
'{V} 이용정보 | {L} 코스·가격·문의 | 몽라운지',
'{L} 지역 {V} | 운영시간·이용정보 | 몽라운지',
'{V} {L} | 코스시간·가격 확인 | 몽라운지',
'{L}에서 보는 {V} | 영업시간·연락 안내 | 몽라운지'
)
...
# {P}에는 "원"까지 들어가며, "부터"는 템플릿에서 한 번만 붙임
$shopFactFormats=@(
'{D} 코스와 {P}부터 확인되는 가격 정보를 포함해 ',
'{P}부터 시작하는 가격과 {D} 코스 정보를 바탕으로 ',
'{D} 이용시간과 {P}부터의 가격 정보를 함께 확인하며 ',
'{D} 코스 구성과 {P}부터의 가격대를 확인한 뒤 ',
'{P}부터 확인되는 가격과 {D} 코스 정보를 참고해 ',
'{D} 코스와 {P}부터의 가격 정보를 포함해 ',
'{D} 시간 구성과 {P}부터 시작하는 가격을 확인하면서 ',
'{P}부터의 가격 정보와 {D} 코스 시간을 함께 살펴보고 '
)
...
foreach($f in $files){
    ...
    try{
        ...
        if($class -eq "지역SEO"){
            ...
            $a=[int]$h[0] % $shopTitleTemplates.Count
            $c=[int]$h[1] % $shopMetaOpen.Count
            $d=[int]$h[2] % $shopMetaClose.Count
            $e=[int]$h[3] % $shopFactFormats.Count
            ...
            if($dur -and $price){
                ...
                $fact="$price부터 확인되는 가격 정보를 포함해 "
                $shopFacts++
            }

            $newTitle=$shopTitleTemplates[$a].Replace('{V}',$v).Replace('{L}',$l)
...
$bad=@($rows | Where-Object {
    $_.NewTitle -match '부터부터|원부터부터|업체 상세정보 .*업체 상세정보' -or
    $_.NewDescription -match '부터부터|원부터부터|\s{2,}'
})

$shopMissingLoc=@($shop | Where-Object {
    $parts=Parse-Shop $_.OldTitle
    if($null -eq $parts){ return $false }
    $loc=$parts[1]
    return ($_.NewTitle -notmatch [regex]::Escape($loc))
})

Write-Host ""
Write-Host "=== TITLE / META v8 최종 미리보기 ==="
...
Write-Host "실제 업체 TITLE 패턴 수:" (@($shop|Select-Object -ExpandProperty TitlePattern -Unique)).Count
Write-Host "실제 업체 META 조합 수:" (@($shop|Select-Object -ExpandProperty MetaPattern -Unique)).Count
Write-Host "업체 TITLE 지역명 누락:" $shopMissingLoc.Count
Write-Host "부터부터/이중공백 의심:" $bad.Count
Write-Host ""
