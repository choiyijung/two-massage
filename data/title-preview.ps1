    ...
    elseif($title -match '^(?<loc>.+?(?:동|읍|면))\s+출장마사지\s*\|\s*(?<parent>.+?)\s+(?:동|읍|면)\s+생활권·지역\s+안내\s*\|\s*몽라운지$'){
        ...
        switch($v){
            ...
            1 {
                ...
                $newDesc="$loc을 중심으로 주거·상업 생활권과 주변 이동 동선을 정리했습니다. 가까운 지역과 이용 범위를 비교하고 위치 선택에 참고할 수 있습니다."
            }
            2 {
                $newTitle="$loc 출장마사지 | $parent 생활권·인접 지역 안내 | 몽라운지"
                $newDesc="$loc에서 오가는 주요 생활권과 주변 지역 정보를 한눈에 확인할 수 있도록 구성했습니다. $parent 내 현재 위치와 인접 지역의 이동 범위를 살펴보세요."
            }
            3 {
