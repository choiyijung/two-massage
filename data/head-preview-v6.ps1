...
$out = Join-Path $dataDir "head-rewrite-preview-v6-$stamp.csv"
...
function Extract-RegionAnchors([string]$html,[string]$primary,[string]$title){
    ...
    foreach($m in [regex]::Matches($html,'(?is)<a\b[^>]*>(.*?)</a>')){
        ...
        $ok=$false
        if($want -eq '구'){
            $ok=($t -match '^[가-힣A-Za-z0-9]+구$')
        }
        elseif($want -eq '시군'){
            $ok=($t -match '^[가-힣A-Za-z0-9]+(?:시|군)$')
        }
        else{
            $ok=($t -match '^[가-힣A-Za-z0-9]+(?:동|읍|면|역)$')
        }
...
function NearbyText([string[]]$arr){
...
$regionMetaPatterns=@(
'{R} 지역에서 {N} 등 주변 생활권의 위치와 이동 범위를 확인할 수 있도록 정리했습니다. 가까운 지역을 비교하고 현재 위치에서 이어지는 생활권을 살펴볼 때 참고할 수 있습니다.',
'{R} 기준으로 {N} 등 인접 지역 정보를 함께 정리했습니다. 생활권 연결 방향과 위치 범위를 비교해 주변 지역을 확인할 때 활용할 수 있습니다.',
'{R} 주변의 {N} 등 연결 지역을 중심으로 위치 정보를 구성했습니다. 인접 생활권과 이동 방향을 비교하고 지역 범위를 확인하는 데 참고할 수 있습니다.',
'{R} 생활권에서 {N} 등 가까운 지역으로 이어지는 범위를 정리했습니다. 주변 위치와 이동 동선을 한눈에 비교할 수 있도록 안내합니다.',
'{R} 위치를 기준으로 {N} 등 주변 지역과의 연결 관계를 확인할 수 있습니다. 인접 생활권과 이동 범위를 비교해 지역 선택 전 참고할 수 있습니다.',
'{R} 지역과 {N} 등 인접 생활권을 함께 확인할 수 있도록 구성했습니다. 가까운 위치와 이동 방향을 비교해 주변 지역을 살펴볼 때 참고하세요.'
)
...
'{V} 코스 안내 | {L} 가격·영업시간 | 몽라운지',
'{L} {V} 상세 안내 | 운영시간·문의 | 몽라운지',
'{V} | {L} 코스 구성과 이용정보 | 몽라운지',
'{L} {V} | 가격·코스·연락 안내 | 몽라운지'
)

$shopMetaPatterns=@(
'{L} 지역의 {V} 이용 정보를 정리했습니다. {FACT}코스와 가격, 영업시간, 전화·문자 문의 방법을 함께 확인할 수 있습니다.',
'{V}의 {L} 이용 안내입니다. {FACT}운영시간과 코스 구성, 가격, 연락 방법을 한 페이지에서 확인할 수 있습니다.',
'{L}에서 {V} 이용 전 확인할 정보를 모았습니다. {FACT}코스별 시간과 가격, 운영시간, 전화·문자 문의 내용을 살펴볼 수 있습니다.',
'{V} 관련 {L} 정보를 확인할 수 있습니다. {FACT}가격과 코스 구성, 영업시간 및 연락 수단을 비교해 이용 전 참고할 수 있습니다.',
'{L} 기준으로 {V} 운영 정보를 정리했습니다. {FACT}코스·가격·영업시간과 전화 또는 문자 문의 방법을 확인할 수 있습니다.',
'{V} 이용을 검토할 때 참고할 {L} 상세 정보입니다. {FACT}코스 구성과 가격, 운영시간, 연락 방법을 한곳에서 확인할 수 있습니다.',
'{L} 생활권에서 확인할 수 있는 {V} 정보를 담았습니다. {FACT}이용 가능한 코스와 가격, 영업시간, 전화·문자 문의 정보를 확인할 수 있습니다.',
'{V}의 코스와 운영 정보를 {L} 기준으로 안내합니다. {FACT}가격과 이용시간, 연락 방법을 확인해 필요한 내용을 미리 비교할 수 있습니다.'
)
...
foreach($f in $files){
    ...
    try{
        ...
        if($class -eq "지역SEO"){
            ...
            $n=NearbyText $near
            if($n -ne "주변 지역"){ $regionWithContext++ }

            $ti=HashIndex $rel $regionTitlePatterns.Count 0
            $mi=HashIndex $rel $regionMetaPatterns.Count 1

            $newTitle=$regionTitlePatterns[$ti].Replace('{R}',$r).Replace('{N}',$n)
            $newMeta=$regionMetaPatterns[$mi].Replace('{R}',$r).Replace('{N}',$n)
            ...
            if($dur -and $price){
                $factSentence="$dur 코스와 $price 확인되는 가격 정보를 포함해 "
                $shopWithFacts++
            } elseif($dur){
                $factSentence="$dur 코스 정보를 포함해 "
                $shopWithFacts++
            } elseif($price){
                $factSentence="$price 확인되는 가격 정보를 포함해 "
                $shopWithFacts++
            }
            ...
            $newMeta=$shopMetaPatterns[$mi].Replace('{V}',$v).Replace('{L}',$l).Replace('{FACT}',$factSentence)
